...
class COCO(BASE):
    def __init__(self, split='train', downsample=1, **kwargs):
        ...
        self.version = '2014'
        self.split = split
        assert self.version in ['2014', '2017']

        if self.version == '2014':
            self.dataset_path = os.path.join(dataset_root, 'coco2014')
        ...
        if self.human_type == 'smpl':
            if self.version == '2014':
                if split == 'train_opt':
                    self.annots_path = os.path.join(self.dataset_path, 'COCO_CHMR_SMPL_OPT.npz')
                else:
                    self.annots_path = os.path.join(self.dataset_path, 'COCO_CHMR_SMPL.npz')
            elif self.version == '2017':
                self.annots_path = os.path.join(self.dataset_path, 'COCO_small_NA_SMPL.npz')
        elif self.human_type == 'smplx':
            assert self.version == '2017'
            self.annots_path = os.path.join(self.dataset_path, 'COCO_NeuralAnnot_SMPLX.npz')
    ...
    def get_raw_data(self, idx):
        ...
        if self.version == '2014':
            img_path = os.path.join(self.dataset_path, 'images', 'train2014', img_name)
        elif self.version == '2017':
