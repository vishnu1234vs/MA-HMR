...
class INSTA(BASE):
    def __init__(self, split='train', downsample=1, **kwargs):
        ...
        self.ds_name = 'insta'
        self.split = split
        self.dataset_path = os.path.join(dataset_root, 'insta')
        if split == 'train_opt':
            self.annots_path = os.path.join(self.dataset_path, 'INSTA_CHMR_SMPL_OPT.npz')
        else:
            self.annots_path = os.path.join(self.dataset_path, 'INSTA_CHMR_SMPL.npz')
